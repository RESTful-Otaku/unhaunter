...
//!
//! Note: contents live in an inner module with `allow(dead_code)` because the
//! upstream `ShaderType` derive (encase) generates dead `check` functions that
//! newer rustc flags as dead code (encase issue #95, bevy issue #20400).
...
pub use imp::{CustomMaterial1, CustomMaterial1Data, CustomMaterial2, UIPanelMaterial};
