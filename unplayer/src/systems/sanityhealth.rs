...
fn lose_sanity(
    ...
    for (mut ps, pos) in &mut qp {
        ...
        let mut sound = 0.0;
        for bpos in bpos.iter_xy_neighbours_nosize(3) {
            sound += bf
                .sound_field
                .get(&bpos)
...
fn visual_health(
    ...
    for player in &qp {
        ...
        for (mut o_uiimage, mut bgcolour, dmg) in &mut qb {
