...
fn test_difficulty_scoring_integration() {
    ...
    let easy_difficulty = CurrentDifficulty(Difficulty::TutorialChapter1.create_difficulty_struct());
...
fn test_movement_calculation_integration() {
    ...
    assert!((distance_moved - 1.0).abs() < 0.001, 
        "Normalized movement should be unit distance, got {}", distance_moved);
}

#[test]
fn test_ghost_difficulty_integration() {
    // Test that ghost systems work with difficulty systems
    for difficulty in [Difficulty::TutorialChapter1, Difficulty::StandardChallenge, Difficulty::MasterChallenge] {
        let config = difficulty.create_difficulty_struct();
        let ghosts = config.ghost_set.as_vec();
        
        assert!(!ghosts.is_empty(), "Difficulty {:?} should have ghosts", difficulty);
        
        for ghost in &ghosts {
            let evidences = ghost.evidences();
            assert_eq!(evidences.len(), 5, "Ghost {:?} should have 5 evidences", ghost);
        }
    }
}
